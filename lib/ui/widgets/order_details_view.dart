    ...
    return Column(
      ...
      children: [
        ...
        if (warningAboutHasDiscount &&
          ...
          const Gap(16),
          const Text(
            'Пользователь заказал товары по скидке, хотите ли принять заказ',
            style: TextStyle(
              fontSize: 15,
        ...
        if (role == Role.courier || isHasCourier) ...[
          const Gap(13),
          if (order.houseAddress != null &&
              order.houseLatitude != null &&
              order.houseLongitude != null) ...[
            Row(
              children: [
                ...
                if (order.houseAddress != null) ...[
                  SizedBox(
                    width: order.houseLatitude != null &&
                            order.houseLongitude != null
                        ? 153
                        : null,
                    child: Text(
                ...
                if (order.houseLatitude != null && order.houseLongitude != null)
                  GestureDetector(
                    onTap: () {
                      openMap(order.houseLatitude!, order.houseLongitude!);
                    },
                    child: Text(
                      'посмотреть в 2Gis',
                      textAlign: TextAlign.center,
          ...
          if (order.subBasement != null) Text('Подъезд - ${order.subBasement}'),
          if (order.floor != null) Text('Этаж - ${order.floor}'),
          if (order.houseAddress != null)
            Text('Номер дома - ${order.houseAddress}'),
          ...
          if (order.shopData != null && role == Role.courier) ...[
            const Gap(30),
            const Text(
              'Адрес магазина',
              style: TextStyle(
            ...
            const Gap(15),
            if (order.shopData!.latitude != null &&
                order.shopData!.longitude != null) ...[
              Row(
                children: [
                  const AppIcon(AppIcons.location, size: 20),
                  const Gap(5),
                  ...
                  //   SizedBox(
                  //     width: order.storeLatitude != null &&
                  ...
                  if (order.shopData!.latitude != null &&
                    ...
                    GestureDetector(
                      onTap: () {
                        openMap(order.shopData!.latitude!,
                            order.shopData!.longitude!);
                      },
                      child: Text(
                        'посмотреть в 2Gis',
                        textAlign: TextAlign.center,
        ...
        Row(
          ...
          children: [
            ...
            if (order.products.isNotEmpty &&
                (role == Role.salesman || isHasCourier) &&
                isShowPrice)
              Text(
                '${order.products.fold(
                  0,
                  (previousValue, element) =>
                      previousValue + (element.quantity.toInt() ?? 0),
                )} шт',
                style: const TextStyle(
                  color: Color(0xFF222222),
                  fontSize: 18,
