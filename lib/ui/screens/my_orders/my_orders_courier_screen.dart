...
import '../fulfillment_order/fulfillment_order_screen.dart';

class MyOrdersCourierScreen extends HookConsumerWidget {
  const MyOrdersCourierScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final userState = ref.watch(userProvider);
    final myOrders = ref.watch(myOrderListProvider);
    final myDishOrders = ref.watch(myDishOrderListProvider);
          ...
          children: [
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return GestureDetector(
                          onTap: () async {
                            ...
                            } else if (userState.role == Role.salesman) {
                              if (order.status == OrderStatus.delivered ||
                                  order.status == OrderStatus.canceled ||
                                  order.status ==
                                      OrderStatus.canceledByClient ||
                                  order.status == OrderStatus.canceledByShop) {
                                showModalBottomSheet(
                                  context: context,
                                  isScrollControlled: true,
                                  backgroundColor: Colors.transparent,
                                ...
                                ref.watch(orderDetailProvider(order.id));
                                await ref
                                    .read(
                                        orderDetailProvider(order.id).notifier)
            ...
            ListView(
              children: [
                ...
                const Center(
                  child: Text(
                    "Здесь будеть отображаться ваши заказы.",
                    style: TextStyle(
                      color: Color(0xFFAEAEAE),
                      fontSize: 16,
                      fontWeight: FontWeight.w400,
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return GestureDetector(
                          onTap: () async {
                            if (userState.role == null) return;
                            if (userState.role == Role.courier) {
                              if (order.status != OrderDishStatus.sending) {
                                showModalBottomSheet(
                                  context: context,
                                  isScrollControlled: true,
                                  backgroundColor: Colors.transparent,
                              ...
                              if (order.status == OrderDishStatus.delivered ||
                                  ...
                                  order.status ==
                                      OrderDishStatus.canceledByClient ||
                                  order.status ==
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        getOrderDishStatus(order.status)
                                            .toLowerCase(),
                                        style: const TextStyle(
                                          fontSize: 14,
                                          color: Color(0xFFA5A5A5),
  ...
  String getOrderDishStatus(OrderDishStatus status) {
    switch (status) {
